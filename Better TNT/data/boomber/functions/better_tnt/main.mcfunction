execute as @e[type=tnt,nbt={Fuse:1s}] at @s unless block ~ ~ ~ minecraft:water unless block ~ ~ ~ minecraft:lava run function boomber:better_tnt/tnt
execute as @e[type=creeper,scores={bb.fuse=29..}] at @s unless block ~ ~ ~ minecraft:water unless block ~ ~ ~ minecraft:lava run function boomber:better_tnt/tnt
